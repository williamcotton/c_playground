...
void print_first_element_of_array(char **array) { printf("%s\n", array[0]); }
...
// given an array and an integer find the difference between the integer and
...
char *insert_char(char *str, char replacement, int index) {
  ...
  snprintf(repl, 2, "%c", replacement); // NOLINT
...
char *itoa(int n) {
  ...
  snprintf(str, 12, "%d", n); // NOLINT
  return str;
}

char *ltoa(long n) {
  char *str = malloc(sizeof(char) * 12);
  snprintf(str, 12, "%ld", n); // NOLINT
  return str;
}

char *lltoa(long long n) {
  char *str = malloc(sizeof(char) * 12);
  snprintf(str, 12, "%lld", n); // NOLINT
  return str;
}